...
class Train(object):
    ...
    def collecting_data_update_model(self, type="training", epoch=0):
        ...
        for uuid in selected_users:
            ...
            while not done:
                data = {"uid": [state[0][1]]}
                for i in range(6):
                    p_r, pnt = self.convert_item_seq2matrix(
                        [[0] + [item[0] for item in state[1] if item[3]["rate"] == i]])
                    data["p" + str(i) + "_rec"] = p_r
                    data["p" + str(i) + "t"] = pnt
                policy = self.fa["model"].predict(self.fa["sess"], data)[0]
                if type == "training":
                    if np.random.random() < 5 * THRESHOLD / (THRESHOLD + self.tau): policy = np.random.uniform(0, 1, (
                    self.args.item_num,))
    ...
    def convert_batch2dict(self, batch, epoch):
        ...
        value[:, 0] = -500
        goals = np.max(value, axis=-1) * np.asarray(dones) * min(self.args.gamma,
                                                                 decay_function(max(end - epoch, 0) + 1)) + goals
