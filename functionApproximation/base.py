...
class basic_model(object):
    ...
    def create_model(cls, config, variable_scope="target", trainable=True, graph_name="DEFAULT", task_index=0):
        ...
        logger.info("CREATE MODEL", config.model, "GRAPH", graph_name, "VARIABLE SCOPE", variable_scope, "jobs", jobs,
                    "job", job, "task_index", task_index)
        cls.CLUSTER = tf.train.ClusterSpec(jobs)
        cls.SERVER = tf.train.Server(cls.CLUSTER, job_name=job, task_index=task_index,
